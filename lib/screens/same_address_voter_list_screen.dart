...
class SameAddressVoterListScreen extends StatefulWidget {
  final String title;
  final int voterId; // ✅ pass voterId dynamically

  const SameAddressVoterListScreen({
    super.key,
    required this.title,
  ...
  @override
  State<SameAddressVoterListScreen> createState() => _BeachScreenState();
}

class _BeachScreenState extends State<SameAddressVoterListScreen> {
  ...
  Future<void> fetchVoters({bool reset = false}) async {
    ...
    final url =
        "https://api.aoinfotech.com/api/GetVotersWithSameAddress?pageindex=$pageIndex&pagesize=$pageSize&voterId=${widget.voterId}&Search_Data=$searchQuery";
